...
NAME_REPLACEMENTS = {
    ...
    re.compile(r'{{sclass\|([a-zA-Z0-9\- ]+)\|([a-zA-Z0-9\- ]+)}}'): r'[[\g<1> class \g<2>]]',
}
...
for name in ['hms', 'hmas', 'hmt', 'sms', 'ss', 'usat', 'uss']:
    ...
    NAME_REPLACEMENTS[
        re.compile(r'{{' + name + r'\|([a-zA-Z0-9\- ]+)\|([a-zA-Z0-9\- ]+)(\|[0-9]+)?}}')
    ] = (r'[[' + name + r' \g<1> (\g<2>)]]')
...
class DykEntryTalkTask(Task):
    ...
    def process_page(self, page: Page):
        ...
        for template in parsed.filter_templates():
            if (template.name.matches('Dyktalk') or template.name.matches('DYK talk')) and (
                not template.has('entry') or len(template.get('entry').value) == 0
            ):
                if year is None:
                ...
                template.name.matches('ArticleHistory') or template.name.matches('Article history')
            ) and (not template.has('dykentry') or len(template.get('dykentry').value) == 0):
                if year is None:
                    ...
                    if ' ' in date:
                        ...
                        print(
                            'Skipping {{ArticleHistory}} on page', page, ", can't parse date", date
                        )
                        continue
